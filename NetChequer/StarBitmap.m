...
u_int32_t PixelIndex(u_int32_t x, u_int32_t y, size_t width)
{
	return (u_int32_t)(x + (y * width));
}
...
void ConvertToMonochromeSteinbertDithering(ARGBPixel * image, size_t width, size_t height, float intensity)
{
	...
	for (int y = 0; y < height; y++)
	{
		if ((y & 1) == 0)
		{
			...
			{
				size_t pixelIndex = PixelIndex(x, y, width);
...
- (NSData *)getImageMiniDataForPrinting:(BOOL)compressionEnable pageModeEnable:(BOOL)pageModeEnable
{
	...
	if (ditheringSupported == true)
	{
		ConvertToMonochromeSteinbertDithering(pixels, (int)width, (int)height, 1.5);
	}
...
//                                  0x1b, 0x57, 0x00, 0x00, 0x00, 0x00, mWidth % 256, mWidth / 256,  height       % 256,  height       / 256,
    ...
    while (totalRowCount < height)
    {
        ...
        if (compressionEnable)
        {
            ...
            command = [SMPort compressRasterData:(int32_t)byteWidth :24 :data :portSettings];
